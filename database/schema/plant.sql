...
CREATE PROCEDURE IF NOT EXISTS `GetPictures`(IN s JSON, IN area POLYGON)
	#NO SQL
	BEGIN
		SET @families = JSON_EXTRACT(s,'$.familySelected');
		SET @genus = JSON_EXTRACT(s,'$.genusSelected');
		...
		CALL CreateTempTables(s); 
	  	...
	  	SELECT Images.image, Images.orientation, Plants.id,
...
&&
*/
CALL GetPictures('{"from":"2021-03-01","to":"2021-03-25"}', NULL);
CALL GetPictures('{"familySelected":["Leguminosae"],"from":"2020-03-15","to":"2021-02-26"}',ST_GEOMFROMTEXT('POLYGON((51.262989030960796 -0.5288915097817393, 51.262989030960796 -0.6696538388833018, 51.21140017256014 -0.6696538388833018,51.21140017256014 -0.5288915097817393,51.262989030960796 -0.5288915097817393))'));
CALL GetPictures('{"from":"2021-02-01","to":"2021-03-30"}',ST_GEOMFROMTEXT('POLYGON((51.262989030960796 -0.5288915097817393, 51.262989030960796 -0.6696538388833018, 51.21140017256014 -0.6696538388833018,51.21140017256014 -0.5288915097817393,51.262989030960796 -0.5288915097817393))'));
CALL GetPictures('{"from":"2021-03-01","to":"2021-03-27"}', (ST_GeomFromText('')));
